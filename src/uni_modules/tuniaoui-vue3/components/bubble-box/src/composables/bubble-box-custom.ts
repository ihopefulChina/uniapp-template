...
export const useBubbleBoxCustomStyle = (props: BubbleBoxProps, showBubble: Ref<boolean>) => {
    const ns = useNamespace('bubble-box');

    // 解析颜色
    const [bgColorClass, bgColorStyle] = useComponentColor(toRef(props, 'bgColor'), 'bg');
    const [borderColorClass, borderColorStyle] = useComponentColor(toRef(props, 'bgColor'), 'border');

    // 选项的类
    const optionsClass = computed<string>(() => {
        const cls: string[] = [ns.e('options'), ns.em('options', props.position), ns.is('show', showBubble.value)];
    ...
    const optionsStyle = computed<CSSProperties>(() => {
        ...
        if (!bgColorClass.value) style.backgroundColor = bgColorStyle.value || 'var(--tn-color-white)';
    ...
    const optionsAuxiliaryElementStyle = computed<CSSProperties>(() => {
        ...
        if (!borderColorClass.value) style.borderColor = borderColorStyle.value || 'var(--tn-color-white)';
    ...
    const optionItemClass = computed<OptionsClassType>(() => {
        return (item: BubbleBoxOptionItemData) => {
            const cls: string[] = [ns.e('option-item'), ns.is('disabled', item.disabled)];
    ...
    const optionItemStyle = computed<OptionsStyleType>(() => {
        return (item: BubbleBoxOptionItemData) => {
            ...
            if (!item.color.class) style.color = item.color.style || 'var(--tn-text-color-primary)';
