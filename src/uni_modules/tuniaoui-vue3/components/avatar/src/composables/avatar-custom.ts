 ...
 * @LastEditTime: 2023-11-22 09:17:47
 * @FilePath: /uniapp_template/src/uni_modules/tuniaoui-vue3/components/avatar/src/composables/avatar-custom.ts
 * @Description:
 *
...
export const useAvatarCustomStyle = (props: AvatarProps, groupIndex: Ref<number>, avatarWidth: Ref<number>) => {
    const ns = useNamespace('avatar');

    const { type, size, shape, bgColor, border, borderColor, shadow, shadowColor, avatarGap } = useAvatarProps(props);

    // 解析背景颜色
    const [bgColorClass, bgColorStyle] = useComponentColor(bgColor, 'bg');

    // 解析边框颜色
    const [borderColorClass, borderColorStyle] = useComponentColor(borderColor, 'border');
    ...
    const avatarStyle = computed<CSSProperties>(() => {
        ...
        // 设置边框颜色
        if (border.value && borderColorStyle.value) style.borderColor = borderColorStyle.value;
