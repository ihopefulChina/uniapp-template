...
export const useSwiperCustomStyle = (props: SwiperProps) => {
    ...
    // 解析颜色
    const [indicatorBgColorClass, indicatorBgColoeStyle] = useComponentColor(toRef(props, 'indicatorBgColor'), 'bg');
    const [indicatorTextColorClass, indicatorTextColorStyle] = useComponentColor(toRef(props, 'indicatorTextColor'), 'text');
    ...
    const swiperStyle = computed<CSSProperties>(() => {
        ...
        if (props.height !== undefined) style.height = formatDomSizeValue(props.height);
    ...
    const indicatorColorClass = computed<IndicatorColorClass>(() => {
        return (active: boolean) => {
            ...
            if (props.indicatorType === 'number') {
                ...
                if (indicatorTextColorClass.value) cls.push(indicatorTextColorClass.value);
            } else {
                if (active) {
                    if (indicatorActiveBgColorClass.value) cls.push(indicatorActiveBgColorClass.value);
                } else {
                    if (indicatorBgColorClass.value) cls.push(indicatorBgColorClass.value);
                }
            }
    ...
    const indicatorColorStyle = computed<IndicatorColorStyle>(() => {
        return (active: boolean) => {
            ...
            if (props.indicatorType === 'number') {
                ...
                if (active) {
                    if (!indicatorActiveBgColorClass.value) style.backgroundColor = indicatorActiveBgColorStyle.value || 'var(--tn-color-white)';
                } else {
                    if (!indicatorBgColorClass.value) style.backgroundColor = indicatorBgColoeStyle.value || 'rgba(0, 0, 0, 0.25)';
                }
            }
