...
export const useFormItemCustomStyle = (props: FormItemProps, hasLabel: Ref<boolean>, isRequired: Ref<boolean>) => {
    ...
    // 标签的宽度
    const labelWidth = computed(() => formatDomSizeValue(props.labelWidth || form?.labelWidth || ''));

    // 标签的位置
    const labelPosition = computed(() => props.labelPosition || form?.labelPosition || 'right');

    // 是否隐藏必填星号
    const hideRequiredAsterisk = computed(() => form?.hideRequiredAsterisk || false);

    // 必填星号的位置
    const requireAsteriskPosition = computed(() => form?.requireAsteriskPosition || 'left');
    ...
    const formItemLabelStyle = computed<CSSProperties>(() => {
        ...
        if (labelPosition.value !== 'top' && labelWidth.value) style.width = labelWidth.value;
        return style;
    });
