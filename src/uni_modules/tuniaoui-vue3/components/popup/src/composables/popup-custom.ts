...
export const usePopupCustomStyle = (props: PopupProps) => {
    ...
    // 内容背景颜色
    const [contentBgColorClass, contentBgColorStyle] = useComponentColor(toRef(props, 'bgColor'), 'bg');
    ...
    const popupContentStyle = computed<CSSProperties>(() => {
        ...
        // 设置背景颜色
        if (contentBgColorStyle.value) style.backgroundColor = contentBgColorStyle.value;
        ...
        if (props.openDirection === 'left' || props.openDirection === 'right') {
            if (props.top) style.height = `calc(100% - ${formatDomSizeValue(props.top, 'px')})`;
        }
