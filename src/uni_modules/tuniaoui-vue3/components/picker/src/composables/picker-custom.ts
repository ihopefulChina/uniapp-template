...
export type PickerOperationBtnStyle = (type: PickerOperationBtnType) => CSSProperties;

export const usePickerCustomStyle = (props: PickerProps) => {
    const ns = useNamespace('picker');

    // 解析颜色
    const [cancelColorClass, cancelColorStyle] = useComponentColor(toRef(props, 'cancelColor'), 'text');
    const [confirmColorClass, confirmColorStyle] = useComponentColor(toRef(props, 'confirmColor'), 'text');
    ...
    const operationBtnClass = computed<PickerOperationBtnClass>(() => {
        return (type: PickerOperationBtnType) => {
            const cls: string[] = [ns.e('operation-btn'), ns.em('operation-btn', type)];
    ...
    const operationBtnStyle = computed<PickerOperationBtnStyle>(() => {
        return (type: PickerOperationBtnType) => {
            ...
            if (type === 'cancel') {
                if (!cancelColorClass.value) style.color = cancelColorStyle.value || 'var(--tn-color-danger)';
            } else if (type === 'confirm') {
                if (!confirmColorClass.value) style.color = confirmColorStyle.value || 'var(--tn-color-primary)';
            }
