...
export const useSlider = (props: SliderProps) => {
    ...
    const sliderValue = ref<SliderValueType>(isEmptyVariableInDefault(props.modelValue, 0));
    ...
    const initSliderValue = () => {
        ...
        if (isArray(val)) {
            ...
            if (val.length === 0) {
                ...
                val = [Math.max(Math.min(val[0], val[1]), props.min), Math.min(Math.max(val[0], val[1], props.min), props.max)];
            }
        } else {
            val = Math.min(Math.max(isEmptyVariableInDefault(val, 0), props.min), props.max);
        }
    ...
    const mode = computed<SliderMode>(() => (isArray(sliderValue.value) ? 'range' : 'single'));
    ...
    const handleInputValue = (value: number, type?: 'min' | 'max') => {
        if (mode.value === 'single') {
            ...
            if (type === 'min') {
                return [Math.min(Math.max(props.min, value), maxValue), (sliderValue.value as number[])![1]];
            } else {
                return [(sliderValue.value as number[])![0], Math.max(Math.min(value, props.max), minValue)];
            }
        }
    };
