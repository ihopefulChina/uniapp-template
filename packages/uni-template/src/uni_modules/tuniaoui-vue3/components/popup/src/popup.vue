...
const { iosDevice, showOverlay, showPopup, visiblePopup, onClickCloseBtn, onClickOverlay } = usePopup(props);
...
<template>
  ...
  >
    <!-- 遮罩层 -->
    <TnOverlay :show="showOverlay" :z-index="overlayZIndex" :opacity="overlayOpacity" @click="onClickOverlay" />
    ...
    <view :class="[popupContentClass]" :style="popupContentStyle">
      ...
      <!-- 关闭按钮 -->
      <view v-if="closeBtn" :class="[ns.e('close-btn'), ns.em('close-btn', closeBtnPosition)]" @tap.stop="onClickCloseBtn">
        <slot name="closeBtn">
          <TnIcon name="close" />
        </slot>
      </view>
...
@use '../../../theme-chalk/src/popup.scss' as *;
</style>
