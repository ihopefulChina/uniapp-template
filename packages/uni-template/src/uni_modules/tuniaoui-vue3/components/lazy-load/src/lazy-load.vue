...
const { componentId, imageStatus, showImage, handleImageLoadedSuccess, handleImageLoadedFailed } = useLazyLoad(props);
...
<template>
  <view :id="componentId" :class="[ns.b(), ns.is('show-image', showImage && imageStatus === 'loaded')]" :style="lazyLoadStyle">
    <!-- 加载中 -->
    ...
    <!-- 正式显示的图片 -->
    <view v-if="showImage && imageStatus !== 'error'" :class="[ns.e('container')]">
      <image
        :class="[ns.e('image'), ns.is('animation', imageStatus === 'loaded' && transition), ns.is('no-animation', imageStatus === 'loaded' && !transition)]"
        :src="src"
        :mode="mode"
...
@use '../../../theme-chalk/src/lazy-load.scss' as *;
</style>
