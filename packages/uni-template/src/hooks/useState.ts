import { ref, computed, watch, Ref, ComputedRef, watchEffect } from 'vue'
...
export function useState<T, C = T>(
  ...
  watch(isControlled, (newVal, oldVal) => {
    if (newVal !== oldVal) {
      console.warn(`WARN: Component changed from ${wasControlled.value ? 'controlled' : 'uncontrolled'}  to ${newVal ? 'controlled' : 'uncontrolled'}`)
  ...
  function setValue(newValue: T | ((prev: T) => T), ...args: any[]) {
    if (typeof newValue === 'function') {
      console.warn('Function callbacks are not supported.')
