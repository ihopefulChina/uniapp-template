...
import { IUniRequestConfig, IUniResponse } from './types/uni-type'
import { IRequestOption } from './types'
import { createInstance } from './helper/createInstance'
import { normalizationUrl } from './core/utils'
export interface IUniInstance extends Omit<UniRequest, 'request'> {
  create: (config?: IRequestOption) => IUniInstance
  <T>(config: IUniRequestConfig): Promise<IUniResponse<T>>
}

function create(config: IRequestOption = {}) {
  const ins = createInstance(config, UniRequest, ['getErrorData']) as IUniInstance
  ins.create = create
  return ins
}
...
export const getToken = merge(async (url: string) => {
