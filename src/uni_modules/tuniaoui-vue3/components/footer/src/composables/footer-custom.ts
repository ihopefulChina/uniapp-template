...
export const useFooterCustomStyle = (props: FooterProps) => {
    ...
    const footerStyle = computed<CSSProperties>(() => {
        ...
        if (props.offsetBottom) style.bottom = formatDomSizeValue(props.offsetBottom);
    ...
    const contentStyle = computed<CSSProperties>(() => {
        ...
        // 设置颜色
        if (!textColorClass.value) style.color = textColorStyle.value || 'var(--tn-text-color-secondary)';
    ...
    const navigatorStyle = computed<FooterNavigatorStyleType>(() => {
        return (item: FooterNavigatorItem) => {
            ...
            // 设置颜色
            if (!item.color.class) style.color = item.color.style || 'var(--tn-color-primary)';
