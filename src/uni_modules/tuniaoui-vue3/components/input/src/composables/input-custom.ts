...
export const useInputCustomStyle = (props: InputProps, validateState: Ref<FormItemValidateStates>, disabled: Ref<boolean>) => {
    ...
    // 解析字数统计颜色
    const [wordLimitColorClass, wordLimitColorStyle] = useComponentColor(toRef(props, 'wordLimitColor'), 'text');
    ...
    const placeholderStyle = computed<string>(() => {
        ...
        if (!isEmpty(props.placeholderStyle)) Object.assign(style, props.placeholderStyle);
    ...
    const inputStyle = computed<CSSProperties>(() => {
        ...
        // 设置边框颜色
        if (props.border && borderColorStyle.value && validateState.value !== 'error') style.borderColor = borderColorStyle.value;
