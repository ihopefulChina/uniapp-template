...
export const useSubsectionCustomStyle = (props: SubsectionProps, activeColor: Ref<string>, sliderRectInfo: Ref<SubsectionSliderRect>) => {
    const ns = useNamespace('subsection');

    // 解析颜色
    const [activeBgColorClass, activeBgColorStyle, updateActiveBgColor] = useComponentColor(activeColor, 'bg');
    const [activeBorderColorClass, activeBorderColorStyle] = useComponentColor(toRef(props, 'activeColor'), 'border');
    watch(
        () => activeColor.value,
        (val) => {
