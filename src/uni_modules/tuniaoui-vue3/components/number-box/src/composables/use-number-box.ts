...
export const useNumberBox = (props: NumberBoxProps) => {
    ...
    const { clearLongPressTimer, handleLongPressEvent: handleOperationEvent } = useLongPress<['minus' | 'plus']>(operationEvent, toRef(props, 'longPress'), props.longPressInterval);
    ...
    const updateNumberBoxValue = (value: number) => {
        ...
        const decimalCount: number = stepValueArray.length > 1 ? stepValueArray?.[1]?.length ?? 0 : 0;
