...
export const MIN_MAX_VALUE = (minValue: number, maxValue: number, currentValue: number) => Math.min(Math.max(minValue, currentValue), maxValue);

export const useDateTimePicker = (props: DateTimePickerProps) => {
    const { emit } = getCurrentInstance()!;

    const { pickerData, minTimeDayjs, maxTimeDayjs, generatePickerData, fillDateTime } = useDateTimeData(props);

    const { handlePickerChangeMinMaxTime } = useHandleMinMaxTime(minTimeDayjs, maxTimeDayjs);
    ...
    const getDateTimeValue = (dateTime: Dayjs) => {
        ...
        if (props.mode === 'yearmonth') return dayjs(`${year}/${month}`).format(props.format || 'YYYY/MM');
        if (props.mode === 'date') return dayjs(`${year}/${month}/${date}`).format(props.format || 'YYYY/MM/DD');
        if (props.mode === 'time') return `${hour}:${minute}:${second}`;
        return dayjs(`${year}/${month}/${date} ${hour}:${minute}:${second}`).format(props.format || innerDefaultDateTimeFormat);
    };
    ...
    const setDefaultPickerSelectValue = (dateTime: Dayjs) => {
        ...
        if (props.mode === 'datetime') pickerSelectData.value = [year, month, date, hour, minute, second];
    };
    // 初始化日期时间选择器
    ...
    const pickerValueChangeEvent = (value: string | number | (string | number)[]) => {
        ...
        if (props.mode === 'date') {
            pickerSelectData.value = [timeValue.year(), timeValue.month() + 1, timeValue.date()];
        }
        if (props.mode === 'time') {
            pickerSelectData.value = [timeValue.hour(), timeValue.minute(), timeValue.second()];
        }
        if (props.mode === 'datetime') {
            pickerSelectData.value = [timeValue.year(), timeValue.month() + 1, timeValue.date(), timeValue.hour(), timeValue.minute(), timeValue.second()];
        }
