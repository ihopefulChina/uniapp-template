...
import { cloneDeep, getProp, isEmptyVariableInDefault, isFunction, isString } from '../../../../utils';
...
export const useFormItemOperation = (props: FormItemProps, slots: Slots) => {
    ...
    const normalizedRules = computed(() => {
        ...
        if (props.required !== undefined) {
            const requiredRules = rules.map((rule, index) => [rule, index] as const).filter(([rule]) => Object.keys(rule).includes('required'));
    ...
    // 是否显示错误信息
    const shouldShowError = computed(() => validateStateDebounced.value === 'error' && props.showMessage && isEmptyVariableInDefault(formContext?.showMessage, true));
    ...
    const onValidationFailed = (error: FormValidateFailure) => {
        ...
        validateMessage.value = errors ? isEmptyVariableInDefault(errors?.[0]?.message, `${props.prop} ${t('common.is_required')}`) : '';
