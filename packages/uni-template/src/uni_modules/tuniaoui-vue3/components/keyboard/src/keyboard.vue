...
const { showKeyboard, carKeyboardLang, popupCloseEvent, keyboardClickEvent, handleBackspaceEvent, clearBackspaceLongPressEvent, carKeyboardSwitchLang } = useKeyboard(props, emits);
const { keyboardData, carKeyboardData } = useKeyboardData(props, carKeyboardLang);
</script>
...
<template>
    ...
    <view class="tn-u-safe-area" :class="[ns.b(), ns.e(mode)]">
      ...
      <template v-if="mode !== 'car'">
        ...
        <view class="right" :class="ns.e('normal-container')">
          ...
          <view class="normal-item confirm" hover-class="keyboard-btn-hover" :hover-stay-time="150" @tap.stop="keyboardClickEvent('confirm', false)"> 确认 </view>
        </view>
      </template>
      ...
      <template v-if="mode === 'car'">
        ...
        <view :class="[ns.e('car-top-operation-bar')]">
          <view class="operation-btn cancel" @tap.stop="keyboardClickEvent('cancel', false)"> 取消 </view>
          <view class="operation-btn confirm" @tap.stop="keyboardClickEvent('confirm', false)"> 确认 </view>
        </view>
        <view :class="[ns.e('car-container')]">
          <!-- 行数据 -->
          <view v-for="(item, index) in carKeyboardData" :key="index" class="column-data">
            <!-- 如果是最后一行，显示切换按钮 -->
            <view v-if="index === 3" class="car-item switch-mode" hover-class="keyboard-btn-hover" :hover-stay-time="150" @tap.stop="carKeyboardSwitchLang">
  ...
  <view v-else :class="[ns.b(), ns.e(mode), ns.m(static ? 'static' : '')]">
    ...
    <template v-if="mode !== 'car'">
      ...
      <!-- 右边操作区域 -->
      <view class="right" :class="ns.e('normal-container')">
        <view class="normal-item delete" hover-class="keyboard-btn-hover" :hover-stay-time="150" @touchstart.stop.prevent="handleBackspaceEvent" @touchend.stop.prevent="clearBackspaceLongPressEvent">
          <TnIcon name="backspace-fill" />
        </view>
        <view class="normal-item confirm" hover-class="keyboard-btn-hover" :hover-stay-time="150" @tap.stop="keyboardClickEvent('confirm', false)"> 确认 </view>
      </view>
...
@use '../../../theme-chalk/src/keyboard.scss' as *;
</style>
