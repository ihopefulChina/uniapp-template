...
export const useBadgeCustomStyle = (props: BadgeProps) => {
    ...
    const badgeContentClass = computed<string>(() => {
        ...
        // 尺寸大小
        if (props.size && sizeType.value === 'inner') cls.push(contentNs.m(props.size as string));
    ...
    const badgeContentStyle = computed<CSSProperties>(() => {
        ...
        // 尺寸大小
        if (props.size && (sizeType.value === 'custom' || contentType.value === 'icon')) style.width = style.height = formatDomSizeValue(props.size);

        // 字体尺寸
        if (props.fontSize) style.fontSize = formatDomSizeValue(props.fontSize);

        // 绝对定位是徽标偏移量
        if (props.absolutePosition.top) style.top = formatDomSizeValue(props.absolutePosition.top);
        if (props.absolutePosition.right) style.right = formatDomSizeValue(props.absolutePosition.right);
