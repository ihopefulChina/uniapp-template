...
const { ns, tabClass, tabStyle, switchTabClass, switchTabStyle } = useSwitchTabCustomStyle(props);
</script>
...
<template>
  <view :class="[switchTabClass]" :style="switchTabStyle">
    ...
    <view :class="[ns.e('tabs')]">
      <view v-for="(tabItem, tabIndex) in tabs" :key="tabIndex" :class="[tabClass(tabIndex)]" :style="tabStyle(tabIndex)" @tap.stop="tabClickEvent(tabIndex)">
...
@use '../../../theme-chalk/src/switch-tab.scss' as *;
</style>
