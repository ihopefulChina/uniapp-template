...
export const useKeyboard = (props: KeyboardProps, emits: SetupContext<KeyboardEmits>['emit']) => {
    ...
    const { handleLongPressEvent: handleBackspaceEvent, clearLongPressTimer: clearBackspaceLongPressEvent } = useLongPress(backspaceEvent, toRef(props, 'longPressDelete'));

    // 车牌键盘中英文切换
    const carKeyboardLang = ref<CarKeyboardLang>(isEmptyVariableInDefault(props.carLang, 'cn'));
