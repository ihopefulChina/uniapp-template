...
export const useInput = (props: InputProps, emits: SetupContext<InputEmit>['emit']) => {
    ...
    // 是否显示状态图标
    const needStatusIcon = computed(() => isEmptyVariableInDefault(form?.statusIcon, false));
    // 校验状态
    const validateState = computed(() => isEmptyVariableInDefault<FormItemValidateStates>(formItem?.validateState, ''));
    // 校验状态图标
    const validateIcon = computed(() => validateState.value && FormValidateIconsMap[validateState.value]);

    // 密码显示密码图标
    const passwordIcon = computed(() => (passwordVisible.value ? 'eye-hide' : 'eye'));
    ...
    // 是否显示字数统计
    const showWordLimit = computed<boolean>(() => props.type === 'textarea' && !!props?.maxlength && !!props?.showWordLimit);
    // 当前的字数
    ...
    const _formatInputText = (value: string) => {
        if (props.type === 'number' || props.type === 'digit') return Number.parseFloat(value);
        return value;
    };
