...
import type { CollapseEmits, CollapseModelValue, CollapseProps } from '../collapse';

export const useCollapse = (props: CollapseProps, emits: SetupContext<CollapseEmits>['emit']) => {
    const { children: items, addChild: addItem, removeChild: removeItem } = useOrderedChildren<CollapseItemContext>();
    // 当前已激活的面板对应的uid
    ...
    const handleItemClick = (uid: number) => {
        ...
        if (accordion) {
            ...
            if (isActive) {
                value = (currentActiveIndex.value as number[]).filter((item) => item !== uidIndex);
            } else {
