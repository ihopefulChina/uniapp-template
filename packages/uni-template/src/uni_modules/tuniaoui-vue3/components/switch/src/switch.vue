...
<template>
  <view
    :class="[
      switchClass,
      {
        [ns.is('content')]: props.activeText || props.activeIcon || props.inactiveText || props.inactiveIcon,
      },
    ]"
    :style="switchStyle"
  ...
  >
    ...
    <view v-if="selected" :class="[ns.e('active-content')]">
      <!-- 激活时的文本 -->
      <view v-if="props.activeText && !props.activeIcon" class="tn-text-ellipsis-1" :class="[ns.em('active-content', 'text')]">
        {{ props.activeText }}
      </view>
      <!-- 激活时的图标 -->
    ...
    <view v-else :class="[ns.e('inactive-content')]">
      <!-- 未激活时的文本 -->
      <view v-if="props.inactiveText && !props.inactiveIcon" class="tn-text-ellipsis-1" :class="[ns.em('inactive-content', 'text')]">
        {{ props.inactiveText }}
      </view>
      <!-- 激活时的图标 -->
      <view v-if="props.inactiveIcon" :class="[ns.em('inactive-content', 'icon')]">
...
@use '../../../theme-chalk/src/switch.scss' as *;
</style>
