 ...
 * @LastEditTime: 2023-11-22 09:41:52
 * @FilePath: /uniapp_template/src/uni_modules/tuniaoui-vue3/components/count-to/src/composables/use-count-to.ts
 * @Description:
 *
...
export const useCountTo = (props: CountToProps, emits: SetupContext<CountToEmits>['emit']) => {
    // 开始值
    const startValue = computed<number>(() => Number(isEmptyVariableInDefault(props.start, 0)));
    // 结束值
    const endValue = computed<number>(() => Number(isEmptyVariableInDefault(props.end, 0)));
    // 判断是往下计数还是往上计数
    const countDown = computed<boolean>(() => startValue.value > endValue.value);

    // 动画执行时间
    const duration = computed<number>(() => isEmptyVariableInDefault(props.duration, 1500));
