...
<template>
    <view>
        ...
        >
            <Navigation :isTab="props?.isTab" :title="props?.showCustomTitle ? '' : props?.title" :type="MMNavigationType.Transparent" :hidenSpace="hidenSpace">
                <template #customTitle>
                    <view class="custrom_nav" :style="{ height: `${systemInfo?.navMenuHeight}px` }">
                        <text v-if="props?.showCustomTitle">{{ title }}</text>
                    </view>
                </template>
            </Navigation>
            <slot name="content" />
