...
type NoticeBarCommonColorClass = (type: 'normal' | 'leftIcon' | 'rightIcon') => string;
type NoticeBarCommonColorStyle = (type: 'normal' | 'leftIcon' | 'rightIcon') => CSSProperties;
...
export const useNoticeBarCommonProps = (props: NoticeBarProps) => {
    ...
    const [leftIconColorClass, leftIconColorStyle] = useComponentColor(toRef(props, 'leftIconColor'), 'text');
    const [rightIconColorClass, rightIconColorStyle] = useComponentColor(toRef(props, 'rightIconColor'), 'text');
    ...
    const commonStyle = computed<NoticeBarCommonColorStyle>(() => {
        return (type: 'normal' | 'leftIcon' | 'rightIcon' = 'normal') => {
            ...
            if (type === 'normal') {
                ...
                if (props.fontSize) style.fontSize = formatDomSizeValue(props.fontSize);
                if (props.leftIconSize) style.fontSize = formatDomSizeValue(props.leftIconSize);
            } else if (type === 'rightIcon') {
                if (!rightIconColorClass.value) {
                    style.color = rightIconColorStyle.value || 'var(--tn-text-color-secondary)';
                }
                if (props.fontSize) style.fontSize = `calc(${formatDomSizeValue(props.fontSize)} * 1.2)`;
                if (props.rightIconSize) style.fontSize = formatDomSizeValue(props.rightIconSize);
            }
