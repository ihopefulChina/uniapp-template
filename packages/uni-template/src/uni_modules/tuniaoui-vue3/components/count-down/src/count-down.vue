...
import { useCountDown, useCountDownCustomStyle, useCountDownSeparatorData } from './composables';

const props = defineProps(countDownProps);
const emits = defineEmits(countDownEmits);

const { day, hour, minute, second, startCountDown, stopCountDown, resetCountDown } = useCountDown(props, emits);
const { countDownClass, countDownStyle, textClass, textStyle, separatorClass, separatorStyle } = useCountDownCustomStyle(props);
...
<template>
  <view :class="[countDownClass]" :style="countDownStyle">
    ...
    <template v-if="second">
      ...
      <view v-if="getSeparatorData(separatorMode, 'second')" class="minute" :class="[separatorClass]" :style="separatorStyle">
...
@use '../../../theme-chalk/src/count-down.scss' as *;
</style>
