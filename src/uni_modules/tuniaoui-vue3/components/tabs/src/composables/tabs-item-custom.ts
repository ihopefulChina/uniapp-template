...
export const useTabsItemCustomStyle = (props: TabsItemProps, isActive: Ref<boolean>) => {
    const ns = useNamespace('tabs-item');

    const tabsContext = inject(tabsContextKey);

    const normalColor = computed<string | undefined>(() => props.color || tabsContext?.color);
    const activeColor = computed<string | undefined>(() => props.activeColor || tabsContext?.activeColor);
    const activeBold = computed<boolean>(() => isEmptyVariableInDefault(tabsContext?.activeBold, true));
    const activeFontSize = computed<string | undefined>(() => props.activeFontSize || tabsContext?.activeFontSize);
