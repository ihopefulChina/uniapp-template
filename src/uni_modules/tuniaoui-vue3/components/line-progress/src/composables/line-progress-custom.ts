...
export const useLineProgressCustomStyle = (props: LineProgressProps) => {
    ...
    const { percent, activeColorClass, activeColorStyle, inactiveColorClass, inactiveColorStyle } = useProgressProps(props);
    ...
    const progressStyle = computed<CSSProperties>(() => {
        ...
        // 设置进度条颜色
        if (!inactiveColorClass.value) style.backgroundColor = inactiveColorStyle.value || 'var(--tn-color-gray-disabled)';
    ...
    const activeProgressStyle = computed<CSSProperties>(() => {
        ...
        // 设置进度条颜色
        if (!activeColorClass.value) style.backgroundColor = activeColorStyle.value || 'var(--tn-color-primary)';
