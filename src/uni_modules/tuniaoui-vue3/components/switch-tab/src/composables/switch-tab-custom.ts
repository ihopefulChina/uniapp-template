...
export const useSwitchTabCustomStyle = (props: SwitchTabProps) => {
    ...
    // 解析颜色
    const [inactiveBgColorClass, inactiveBgColorStyle] = useComponentColor(toRef(props, 'inactiveBgColor'), 'bg');
    const [activeBgColorClass, activeBgColorStyle] = useComponentColor(toRef(props, 'activeBgColor'), 'bg');
    const [inactiveTextColorClass, inactiveTextColorStyle] = useComponentColor(toRef(props, 'inactiveTextColor'), 'text');
    const [activeTextColorClass, activeTextColorStyle] = useComponentColor(toRef(props, 'activeTextColor'), 'text');
    ...
    const tabClass = computed<tabClass>(() => {
        return (index: number) => {
            const cls: string[] = [ns.e('tab'), ns.is('active', index === props.modelValue)];
