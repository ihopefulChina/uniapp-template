...
const { componentContentId, showArrow, isActive, componentHeight, collapseItemClick } = useCollapseItem(props);
...
<template>
  <view :class="[ns.b(), ns.is('active', isActive), ns.is('disabled', disabled)]" :style="{ height: componentHeight }">
    <!-- 标题 -->
    <view :class="[ns.e('title')]" @tap.stop="collapseItemClick">
      ...
      <view v-if="showArrow" class="arrow" :class="[arrowClass]" :style="arrowStyle">
        <TnIcon name="right" />
      </view>
    </view>
