...
export const useImageUpload = (props: ImageUploadProps) => {
    ...
    const { chooseImage, uploadProcess, checkFileSizeAndExtension, showErrorTips } = useUploadHandleFunction(props);
    ...
    // 处理上传事件
    const handleUploadEvent = (item: ImageUploadListItem, index: number, uploadSingle = false) => {
        uploadProcess(item)
            .then((res) => {
                if (res) {
    ...
    const uploadFile = (startIndex: number, uploadSingle = false) => {
        ...
        const isBeforeUploadPromiseOrBoolean = [isPromise(shouldUpload), isBoolean(shouldUpload)].includes(true);
    ...
    const retryAllUpload = () => {
        // 查找出第一张上传失败的图片
        const firstFailedFileIndex = fileList.value.findIndex((item) => item.status === 'failed');
        ...
        uni.showModal({
            ...
            success: (res) => {
                if (res.confirm) {
                    ...
                    const isShouldRemovePromiseOrBoolean = [isPromise(shouldRemove), isBoolean(shouldRemove)].includes(true);
