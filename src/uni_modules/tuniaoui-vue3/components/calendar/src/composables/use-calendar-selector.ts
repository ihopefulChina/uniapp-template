...
export const useCalendarSelector = (currentDateCount: Ref<number>, mode: CalendarMode) => {
    ...
    // 日期容器的高度
    const dateContainerHeight = computed<number>(() => Math.ceil(currentDateCount.value / 7) * singleDateItemHeight.value);

    let initCount = 0;
    // 获取单个日期容器的信息
    const getDateItemComponentRectInfo = async () => {
        try {
            const rectInfo = await getSelectorNodeInfo(`#${calendarId} .tn-calendar__data__date`);
