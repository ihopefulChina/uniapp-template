...
export const useCheckboxCustomStyle = (props: CheckboxProps) => {
    ...
    const { activeColor, disabled, maxDisabled, size, border, checkedShape } = useCheckboxCommonProps(props);

    // 解析颜色
    const [bgColorClass, bgColorStyle] = useComponentColor(activeColor, 'bg');
    const [textColorClass, textColorStyle] = useComponentColor(activeColor, 'text');
    const [borderColorClass, borderColorStyle] = useComponentColor(activeColor, 'border');
    ...
    const checkboxStyle = computed<selectedStyle>(() => {
        return (selected: boolean) => {
            ...
            // 设置激活时的颜色
            if (selected) {
                if (border.value && !borderColorClass.value) style.borderColor = borderColorStyle.value || 'var(--tn-color-primary)';
