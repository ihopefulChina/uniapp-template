...
const { swiperData, currentSwiperIndex, swiperCount, swiperChangeHandle, itemClickHandle } = useSwiper(props, emits);
const { ns, swiperStyle, indicatorColorClass, indicatorColorStyle } = useSwiperCustomStyle(props);
</script>
...
<template>
  <view :class="[ns.b()]" :style="swiperStyle" @tap.stop="itemClickHandle">
    <view :class="[ns.e('wrapper')]">
      ...
      >
        <swiper-item v-for="(item, index) in swiperData" :key="index" :class="[ns.e('swiper-item')]">
          <slot :active="index === currentSwiperIndex" :data="item" :index="index" />
        </swiper-item>
      </swiper>
    </view>
    <!-- 指示器 -->
    <view v-if="indicator" :class="[ns.e('indicator'), ns.em('indicator', indicatorPosition)]">
      <!-- 矩形 -->
      <view v-if="indicatorType === 'line'" :class="[ns.e('indicator-line')]">
        <view class="indicator-wrapper" :class="[indicatorColorClass(false)]" :style="indicatorColorStyle(false)">
          <view
            class="active-block"
      ...
      <view v-if="indicatorType === 'dot'" :class="[ns.e('indicator-dot')]">
        <view class="indicator-wrapper">
          <view
            ...
            :key="i"
            class="dot-item"
            :class="[indicatorColorClass(i === currentSwiperIndex), { active: i === currentSwiperIndex }]"
            :style="indicatorColorStyle(i === currentSwiperIndex)"
          />
        </view>
      </view>
      <!-- 数值 -->
      <view v-if="indicatorType === 'number'" :class="[ns.e('indicator-number')]">
        <view class="indicator-wrapper" :class="[indicatorColorClass(false)]" :style="indicatorColorStyle(false)">
...
<style lang="scss">
@use '../../../theme-chalk/src/swiper.scss' as *;
</style>
