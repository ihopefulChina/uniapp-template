...
export const useSwiper = (props: SwiperProps, emits: SetupContext<SwiperEmits>['emit']) => {
    // 当前选中的SwiperItem索引
    const currentSwiperIndex = ref<number>(isEmptyVariableInDefault(props?.modelValue, 0));
    ...
    const swiperData = computed<any[]>(() => {
        ...
        if (props.blankCount) return Array.from({ length: props.blankCount }).map((_, i) => i);
        return [];
    });
    ...
    const swiperChangeHandle = (event: any) => {
        ...
        if (props.modelValue === undefined || props.modelValue === 0) currentSwiperIndex.value = current;
