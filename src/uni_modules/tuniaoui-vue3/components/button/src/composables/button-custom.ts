...
export const useButtonCustomStyle = (props: ButtonProps) => {
    ...
    // 解析阴影颜色
    const [shadowColorClass, shadowColorStyle] = useComponentColor(toRef(props, 'shadowColor'), 'shadow');
    // 按钮动态类
    ...
    const buttonStyle = computed<CSSProperties>(() => {
        ...
        if (props.height && props.shape !== 'circle') style.height = formatDomSizeValue(props.height);
        ...
        // 设置阴影颜色
        if (props.shadow && shadowColorStyle.value) style.boxShadow = shadowColorStyle.value;
