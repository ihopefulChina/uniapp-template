...
export const useActionSheet = () => {
    ...
    const popupCloseEvent = () => {
        ...
        const isPromiseOrBoolean = [isPromise(shouldCancel), isBoolean(shouldCancel)].includes(true);
    ...
    const actionClickEvent = (index: number) => {
        ...
        const shouldSelect = options.select(index, options?.actions?.[index]?.value as any);
        const isPromiseOrBoolean = [isPromise(shouldSelect), isBoolean(shouldSelect)].includes(true);
        if (!isPromiseOrBoolean) {
