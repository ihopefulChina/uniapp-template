...
export const useSwipeActionOptions = (props: SwipeActionItemProps) => {
    ...
    const options = computed<OptionsData[]>(() => {
        return props.options.map((item) => {
            ...
            const [textColorClass, textColoeStyle] = useComponentColor(textColor, 'text');
    ...
    const optionColorStyle = computed<OptionStyleValue>(() => {
        return (index: number) => {
            ...
            if (option) {
                if (!option.textColor.class) style.color = option.textColor.style || 'var(--tn-color-white)';
                if (!option.bgColor.class) style.backgroundColor = option.bgColor.style || 'var(--tn-color-primary)';
            }
