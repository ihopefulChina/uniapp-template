...
export const useSearchBoxCustomStyle = (props: SearchBoxProps) => {
    ...
    // 解析颜色
    const [textColorClass, textColorStyle] = useComponentColor(toRef(props, 'textColor'), 'text');
    const [borderColorClass, borderColorStyle] = useComponentColor(toRef(props, 'borderColor'), 'border');
    const [placeholderColorClass, placeholderColorStyle] = useComponentColor(toRef(props, 'placeholderColor'), 'text');
    ...
    const searchBoxClass = computed<string>(() => {
        const cls: string[] = [ns.b(), ns.m(props.shape), ns.is('no-search-btn', !props.searchButton), ns.is('disabled', props.disabled)];
    ...
    const searchBoxStyle = computed<CSSProperties>(() => {
        ...
        if (!textColorClass.value) style.color = textColorStyle.value || 'var(--tn-text-color-primary)';
    ...
    const placeholderClass = computed<string>(() => {
        const cls: string[] = [ns.e('placeholder'), ns.em('placeholder', props.textAlign)];
    ...
    const placeholderStyle = computed<CSSProperties>(() => {
        ...
        if (!placeholderColorClass.value) style.color = placeholderColorStyle.value || 'var(--tn-text-color-secondary)';
    ...
    const searchButtonClass = computed<string>(() => {
        ...
        if (searchButtonTextColorClass.value) cls.push(searchButtonTextColorClass.value);
    ...
    const searchButtonStyle = computed<CSSProperties>(() => {
        ...
        if (!searchButtonBgColorClass.value) style.backgroundColor = searchButtonBgColorStyle.value || 'var(--tn-color-primary)';
