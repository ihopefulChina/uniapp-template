...
export const useLoadingCustomStyle = (props: LoadingProps) => {
    ...
    // 解析颜色
    const [colorClass, colorStyle, updateColor] = useComponentColor(toRef(props, 'color'), 'bg');
    ...
    const loadingClass = computed<string>(() => {
        ...
        // 设置尺寸
        if (props.size && sizeType.value === 'inner') cls.push(ns.m(props.size as string));
    ...
    const loadingStyle = computed<CSSProperties>(() => {
        ...
        // 设置尺寸
        if (props.size && sizeType.value === 'custom') style.width = style.height = formatDomSizeValue(props.size);
