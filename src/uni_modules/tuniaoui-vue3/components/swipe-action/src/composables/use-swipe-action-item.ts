...
export const useSwipeActionItem = (props: SwipeActionItemProps) => {
    ...
    // 当前item的菜单是否为打开状态
    const isOptionsOpen = computed<boolean>(() => swipeActionContext!.activeUid.includes(uid));

    // 点击options后是否自动关闭
    const autoClose = computed<boolean>(() => isEmptyDoubleVariableInDefault(props?.autoClose, swipeActionContext?.autoClose, true));

    // 禁止滑动
    const disabledSwipe = computed<boolean>(() => props.disabled || !props.options?.length);
    watch(
        () => disabledSwipe.value,
        (val) => {
    ...
    const onTouchMove = (event: any) => {
        ...
        if (contentRightDistance.value > optionsMenuWidth.value) contentRightDistance.value = optionsMenuWidth.value;
    };
