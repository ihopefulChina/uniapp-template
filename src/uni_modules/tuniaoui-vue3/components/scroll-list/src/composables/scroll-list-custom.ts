...
export const useScrollListCustomStyle = (props: ScrollListProps) => {
    ...
    // 解析颜色
    const [indicatorColorClass, indicatorColorStyle] = useComponentColor(toRef(props, 'indicatorColor'), 'bg');
    const [indicatorBlockColorClass, indicatorBlockColorStyle] = useComponentColor(toRef(props, 'indicatorBlockColor'), 'bg');
    ...
    const indicatorStyle = computed<CSSProperties>(() => {
        ...
        if (props.indicatorWidth) style.width = formatDomSizeValue(props.indicatorWidth, 'px');
    ...
    const indicatorBlockStyle = computed<(distance: number) => CSSProperties>(() => {
        return (distance: number) => {
            ...
            if (props.indicatorBlockWidth) style.width = formatDomSizeValue(props.indicatorBlockWidth, 'px');
