...
import type { WeekCalanderDateStatus, WeekCalendarData, WeekCalendarItem } from '../types';
import { useLocale } from '~/hooks';

export const useWeekCalendar = (props: WeekCalendarProps, emits: SetupContext<WeekCalendarEmits>['emit']) => {
    const { t } = useLocale();
    // 当前日期的Dayjs
    const currentDateDayjs = dayjs();

    const { componentDateItemId, dateItemContainerHeight } = useWeekCalendarSelector();

    // 当前日期的年份
    const currentYear = computed<number>(() => (props.year ? Number(props.year) : currentDateDayjs.year()));
    // 当前日期的月份
    const currentMonth = computed<number>(() => (props.month ? Number(props.month) : currentDateDayjs.month() + 1));
    // 最小可选日期
    const minDate = computed<number>(() =>
        props.minDate ? Number(props.minDate) : currentDateDayjs.year() === currentYear.value && currentDateDayjs.month() + 1 === currentMonth.value ? currentDateDayjs.date() : 1
    );
    // 最大可选日期
    const maxDate = computed<number>(() => (props.maxDate ? Number(props.maxDate) : currentDateDayjs.daysInMonth()));
    ...
    // 当前激活的日期
    const activeDate = computed<number>(() => isEmptyVariableInDefault(props.modelValue, 0));
    // 当前选中的轮播位置
    ...
    const generateWeekCalendarData = () => {
        ...
        for (let i = 1; i <= dates; i++) {
            let status: WeekCalanderDateStatus = i >= minDate.value && i <= maxDate.value ? 'normal' : 'disabled';
    ...
    const dateItemClick = (item: WeekCalendarItem) => {
        if (item.status === 'active' || item.status === 'disabled' || item.date === 0) return;
