...
export const useIcon = (props: IconProps) => {
    ...
    // 解析图标颜色
    const [colorClass, colorStyle, updateColor] = useComponentColor(toRef(props, 'color'), 'text');
    // 解析透明图标背景
    ...
    // 判断是图片还是图鸟内置的图标
    const isImg = computed<boolean>(() => !!props?.name && props.name.includes('/'));
    ...
    const iconStyle = computed<CSSProperties>(() => {
        ...
        if (isImg.value) {
            ...
            // 设置图标大小
            if (sizeType.value === 'custom' && props.size) style.fontSize = formatDomSizeValue(props.size);
        }
        // 设置垂直方向上的偏移量
        if (props.offsetTop) style.transform = `translateY(${formatDomSizeValue(props.offsetTop)})`;
