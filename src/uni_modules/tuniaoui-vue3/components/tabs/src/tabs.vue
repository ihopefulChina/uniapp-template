...
const { tabItems, componentId, barComponentId, barOffsetLeft, scrollLeft, showBar } = useTabs(props);
const { ns, tabsClass, tabsStyle, barClass, barStyle } = useTabsCustomStyle(props);
</script>

<template>
    <view :id="componentId" :class="[tabsClass]" :style="tabsStyle">
        <!-- 距离顶部的距离占位 -->
        <view v-if="offsetTop" :class="[ns.e('top-placeholder')]" :style="{ height: `${offsetTop}px` }" />
        <!-- 内容区域 -->
        <scroll-view
        ...
        >
            <view :class="[ns.e('container'), ns.is('scroll', scroll), ns.is('no-bar', !showBar)]">
                <slot />
                <!-- 滑块 -->
                <view
                    v-if="bar || $slots.bar"
