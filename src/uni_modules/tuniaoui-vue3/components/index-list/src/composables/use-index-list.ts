...
import type { IndexListEmits, IndexListKeys, IndexListProps } from '../index-list';
import type { IndexListData, IndexListDataItem, KeyListRectInfo } from '../types';

export const useIndexList = (props: IndexListProps, emits: SetupContext<IndexListEmits>['emit']) => {
    ...
    const contentContainerHeight = computed<number>(() => props.height || systemScreenInfo.height - props.stickyOffsetTop);
    ...
    const getContentItemNodeInfo = async () => {
        try {
            const contentNodeInfos = await getSelectorNodeInfos(`.${componentContentClass}`);
    ...
    const getKeyListNodeInfo = async () => {
        try {
            const keyListNodeInfo = await getSelectorNodeInfo(`#${componentKeyListId}`);
            const keyListItemNodeInfo = await getSelectorNodeInfos(`#${componentKeyListId} .key-value`);
    ...
    const currentTouchKeyValue = computed<string>(() => keysData.value[currentTouchKeyIndex.value]);
    // 设置当前滑动、点击索引的值
    const updateKeyListIndexValue = () => {
        // 判断当前滑动位置在哪里
        let index = -1;
        // #ifndef APP-PLUS || MP-ALIPAY
        index = keyListItemRectInfo.findLastIndex((item) => item.top < keyListTouchCurrentY.value);
        // #endif
