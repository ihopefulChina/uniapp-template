...
import type { PickerDataType, PickerMode, PickerProps, PickerValueType } from '../picker';
...
export const usePicker = (props: PickerProps) => {
    ...
    // 生成指定格式的数据
    const _generateData = (data: PickerDataType): Pick<PickerDataItem, 'label' | 'value' | 'originalData'> => {
        if (isObject(data)) {
            return {
    ...
    // 更新/生成级联选择器的数据
    const _generateOrUpdateCascadeData = (data: any[], generateIndex = 1, defaultValue: Array<string | number> = []) => {
        ...
        if (defaultValue.length) {
            childrenIndex = pickerData.value[generateIndex - 1].findIndex((item) => item.value === defaultValue[generateIndex - 1]);
        ...
        if (data[childrenIndex] && Object.prototype.hasOwnProperty.call(data[childrenIndex], props.childrenKey)) {
            _generateOrUpdateCascadeData(data[childrenIndex][props.childrenKey] as PickerDataItem[], generateIndex + 1, defaultValue);
        }
    };
                ...
                indexValue = pickerData.value.map((item, index) => {
                    ...
                    if (!(props.modelValue as (string | number)[])[index]) pickerIndex = 0;
                        ...
                        pickerIndex = item.findIndex((childItem) => {
                            return childItem.value === (props.modelValue as (string | number)[])[index];
                        });
                    }
                    return ~pickerIndex ? pickerIndex : 0;
                });
            } else {
                indexValue = [pickerData.value[0].findIndex((item) => item.value === props.modelValue)];
            }
        }
    ...
    const splitUserPickerData = () => {
        ...
        if (isArray(data[0])) {
            // 多选选择器
            pickerMode = 'multiple';
            pickerData.value = (data as PickerDataType[][]).reduce((prev: PickerData, cur: Array<PickerDataType>) => {
            ...
            // 级联选择器
            pickerMode = 'cascade';
            _generateOrUpdateCascadeData(data as PickerDataItem[], 1, props.modelValue as Array<string | number>);
        } else {
            // 单列选择器
            pickerMode = 'signle';
    ...
    const _getCurrentPickerValue = (): PickerValueType => {
        if (pickerMode === 'signle' && !isArray((props.data as any[])[0])) {
            ...
            return pickerIndex.map((item, index) => isEmptyVariableInDefault(pickerData.value[index][item]?.value, 0));
        }
    };
    ...
    const _getCurrentPickerOriginData = (): any => {
        if (pickerMode === 'signle' && !isArray((props.data as any[])[0])) {
            ...
            const pickerIndex = cloneDeep(currentPickerIndex.value);
            pickerIndex.splice(pickerData.value.length);
            return pickerIndex.map((item, index) => isEmptyVariableInDefault(pickerData.value[index][item]?.originalData, undefined));
        }
    };
    ...
    const _generatePickerViewData = (val: any) => {
        ...
        if (pickerMode === 'cascade') {
            _generateOrUpdateCascadeData(props.data as PickerDataItem[], 1, val as Array<string | number>);
        }
        nextTick(() => {
            initDefaultPickerIndex();
        });
    ...
    const pickerViewChangeEvent = (e: any) => {
        ...
        // 比较上一次的值，判断是那一列发生了改变
        let changePickerColumnIndex = currentPickerIndex.value.findIndex((item, index) => item !== e.detail.value[index]);
        ...
        if (pickerMode === 'cascade') {
            ...
            if (data[pickerIndex] && Object.prototype.hasOwnProperty.call(data[pickerIndex], props.childrenKey)) {
                _generateOrUpdateCascadeData(data[pickerIndex][props.childrenKey] as PickerDataItem[], changePickerColumnIndex + 2);
