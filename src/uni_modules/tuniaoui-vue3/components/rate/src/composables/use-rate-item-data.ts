...
const useRateItemData = (props: RateProps) => {
    ...
    const generateRateItemData = () => {
        const { max, inactiveIcon, activeIcon, inactiveColor, activeColor, customData } = props;
        ...
        for (let i = 0; i < max; i++) {
            ...
            // 解析颜色值
            const [inactiveColorClass, inactiveColorStyle] = useComponentColor(toRef(configData, 'inactiveColor'), 'text');
            const [activeColorClass, activeColorStyle] = useComponentColor(toRef(configData, 'activeColor'), 'text');
