...
export const useStickyCustomStyle = (props: StickyProps, suppoerCSSSticky: Ref<boolean | null>, stickyDistance: Ref<number>, stickyStatus: Ref<boolean>, stickyRectInfo: Ref<StickyRectInfo>) => {
    ...
    const stickyStyle = computed<CSSProperties>(() => {
        ...
        if (props.enabled) {
            ...
            if (suppoerCSSSticky.value) {
                ...
                style.height = stickyStatus.value ? formatDomSizeValue(stickyRectInfo.value.height, 'px') : 'auto';
            }
        } else {
