...
export const useDateTimeData = (props: DateTimePickerProps) => {
    ...
    // 获取日期时间的最大/小值
    const getTimeMaxMinValue = (currentValue: TimeOptions | Dayjs, maxMinValue: TimeOptions | Dayjs, defaultValue: TimeOptions): TimeOptions => {
        ...
        if (currentValue.toString() === '[object Object]') {
            ({ year: currentYear, month: currentMonth, date: currentDate, hour: currentHour, minute: currentMinute } = currentValue as TimeOptions);
        } else {
        ...
        if (maxMinValue.toString() === '[object Object]') {
            ({ year: maxMinYear, month: maxMinMonth, date: maxMinDate, hour: maxMinHour, minute: maxMinMinute, second: maxMinSecond } = maxMinValue as TimeOptions);
        } else {
    ...
    const generatePickerData = (value: Dayjs) => {
        ...
        monthColumnData.value = generateRangeData(minMonth, maxMonth).map((month) => {
            return {
                label: `${String(month).padStart(2, '0')}${t('common.month')}`,
                value: month,
            };
        });
        dayColumnData.value = generateRangeData(minDay, maxDay).map((day) => {
            return {
                label: `${String(day).padStart(2, '0')}${t('common.day')}`,
                value: day,
            };
        });
    ...
    const pickerData = computed<PickerData>(() => {
        ...
        if (props.mode === 'yearmonth') result.push(yearColumnData.value, monthColumnData.value);
        if (props.mode === 'date') result.push(yearColumnData.value, monthColumnData.value, dayColumnData.value);
        if (props.mode === 'time') result.push(hourColumnData.value, minuteColumnData.value, secondColumnData.value);
        if (props.mode === 'datetime') result.push(yearColumnData.value, monthColumnData.value, dayColumnData.value, hourColumnData.value, minuteColumnData.value, secondColumnData.value);
