...
export const useSliderCustomStyle = (props: SliderProps, sliderValue: Ref<SliderValueType>, mode: Ref<SliderMode>) => {
    const ns = useNamespace('slider');

    const { size, disabled } = useSliderCommonProps(props);

    // 解析颜色
    const [activeBgColorClass, activeBgColorStyle] = useComponentColor(toRef(props, 'activeColor'), 'bg');
    const [inactiveBgColorClass, inactiveBgColorStyle] = useComponentColor(toRef(props, 'inactiveColor'), 'bg');

    // 滑块的位置
    const sliderBarPosition = computed<[string, string]>(() => {
        if (mode.value === 'single') {
            return [`${(((sliderValue.value as number) - props.min) / (props.max - props.min)) * 100}%`, '0'];
        } else {
            return [`${(((sliderValue.value as number[])[0] - props.min) / (props.max - props.min)) * 100}%`, `${(((sliderValue.value as number[])[1] - props.min) / (props.max - props.min)) * 100}%`];
        }
    });
    ...
    const sliderStyle = computed<CSSProperties>(() => {
        ...
        // 设置滑动条的高度
        if (props.sliderHeight) style.height = formatDomSizeValue(props.sliderHeight);

        // 设置未激活时的颜色
        if (!inactiveBgColorClass.value) style.backgroundColor = inactiveBgColorStyle.value || 'var(--tn-color-grey-light)';
    ...
    const activeSliderStyle = computed<CSSProperties>(() => {
        ...
        // 设置激活时的颜色
        if (!activeBgColorClass.value) style.backgroundColor = activeBgColorStyle.value || 'var(--tn-color-primary)';
    ...
    const sliderBarStyle = computed<(type: 'min' | 'max') => CSSProperties>(() => {
        return (type) => {
            ...
            // 设置滑块的尺寸
            if (props.sliderBarSize) style.width = style.height = formatDomSizeValue(props.sliderBarSize);
