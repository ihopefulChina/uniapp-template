...
import type { RegionPickerEmits, RegionPickerModelValueType, RegionPickerProps } from '../region-picker';

type RegionModelValueMode = 'code' | 'name';

export const useRegionPicker = (porps: RegionPickerProps, emits: SetupContext<RegionPickerEmits>['emit']) => {
    // 列表数据
    ...
    const getCodeByNames = (names: string[]): string[] => {
        ...
        do {
            ...
            const regionItem: RegionDataItem = data.find((item) => item.name === name) || data[0];
            if (regionItem) {
    ...
    const getNameByCodes = (codes: string[]): string[] => {
        ...
        do {
            ...
            const regionItem: RegionDataItem = data.find((item) => item.code === code) || data[0];
            if (regionItem) {
    ...
    // picker发生改变回调
    const handlePickerChangeEvent = (value: string | number | (string | number)[], index: number, item: any) => {
    ...
    // picker确认事件
    const handlePickerConfirmEvent = (value: string | number | (string | number)[], item: any) => {
