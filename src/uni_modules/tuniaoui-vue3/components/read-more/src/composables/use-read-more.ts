...
export const useReadMore = (props: ReadMoreProps, emits: SetupContext<ReadMoreEmits>['emit']) => {
    ...
    // 是否显示操作区域
    const showOperationArea = computed<boolean>(() => !expandStatus.value || (expandStatus.value && props.showFold));
    // 操作区域高度
    ...
    const toggleExpandStatus = () => {
        if (expandStatus.value) {
            ...
            const isPromiseOrBoolean = [isPromise(shouldExpand), isBoolean(shouldExpand)].includes(true);
