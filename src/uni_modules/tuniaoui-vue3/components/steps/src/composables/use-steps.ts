...
export const useSteps = (props: StepsProps) => {
    ...
    const { children: items, addChild, removeChild: removeItem } = useOrderedChildren<StepContext>();
    ...
    const addItem = (item: StepContext) => {
        addChild(item);
        if (!activeUidList.value.length && props.modelValue !== undefined && props.modelValue >= 0) {
