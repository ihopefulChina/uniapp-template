...
// 判断复选框组中是否包含某个值
const hasLabelInGroup = (groupValue: CheckboxValueType[], label: CheckboxValueType) => groupValue.includes(label);

export const useCheckbox = (props: CheckboxProps) => {
    const { emit } = getCurrentInstance()!;

    const { checkboxGroup, disabled, maxDisabled, labelDisabled } = useCheckboxCommonProps(props);
