...
export default function useUploadHandleFunction(props: ImageUploadProps) {
    ...
    watch(
        ...
        (val) => {
            if (props.showErrorTips) {
                if (val) uni.showLoading({ title: t('common.uploading') });
    ...
    const uploadProcess = (item: ImageUploadListItem): Promise<boolean> => {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (customUploadHandler) {
                ...
                const isUploadHandlePromiseOrString = [isPromise(uploadHandlerResult), isString(uploadHandlerResult)].includes(true);
                if (!isUploadHandlePromiseOrString) {
                    console.error('[TnImageUpload]自定义上传处理函数必须返回Promise和String');
                ...
                if (isPromise(uploadHandlerResult)) {
                    uploadHandlerResult
                        ...
                        .catch((err) => {
                            ...
                            reject(err?.errMsg || t('common.error_uploading_file'));
                        })
                        .finally(() => {
                ...
                const task = uni.uploadFile({
                    ...
                    success: (res: UniApp.UploadFileSuccessCallbackResult) => {
                        ...
                        if (customUploadCallback) {
                            ...
                            const isCustomUploadCallbackPromiseOrString = [isPromise(customUploadCallbackResult), isString(customUploadCallbackResult)].includes(true);
                            if (!isCustomUploadCallbackPromiseOrString) {
                                console.error('[TnImageUpload]自定义上传回调函数必须返回Promise和String');
                            ...
                            if (![200, 201, 204].includes(statusCode)) {
                                ...
                                reject(res?.errMsg || t('common.error_uploading_file'));
                                return;
                            } else {
                                ...
                                if (data.code === 200 && data.data.errCode === 0) {
                                    ...
                                    console.error('[TnImageUpload]上传文件发生错误', res);
                                    reject(isEmptyVariableInDefault(data?.message, data?.msg || t('common.error_uploading_file')));
                                }
                            }
                        }
                    },
    ...
    const checkFileSizeAndExtension = (files: Array<ImageUploadFile>) => {
        ...
        return files.filter((item) => {
            ...
            // #endif
            // #ifndef H5
            fileExt = (item as UniApp.ChooseImageSuccessCallbackResultFile).path.replace(extReg, '').toLowerCase();
            // #endif
