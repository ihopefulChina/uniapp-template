...
export const useNavbarCustomStyle = (props: NavbarProps, backButtonType: Ref<NavbackButtonType>, hasRightOperation: Ref<boolean>) => {
    ...
    // 状态栏的高度
    const navbarHeight = computed<string>(() => (props.height ? formatDomSizeValue(props.height) : `${navBarInfo.height}px`));
    ...
    const rightOperationStyle = computed<CSSProperties>(() => {
        ...
        // 设置宽度
        if (props.rightOperationWidth) style.width = formatDomSizeValue(props.rightOperationWidth);
