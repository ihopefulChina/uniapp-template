...
<template>
  <view :class="[ns.b()]" :style="{ height: `${contentContainerHeight}px` }">
    <!-- 列表内容 -->
    <scroll-view :class="[ns.e('scroll-view')]" scroll-y :scroll-top="scrollViewTopValue">
      <view
        :style="{
          paddingTop: `${stickyOffsetTop || 0}px`,
        }"
      >
        <view v-for="(item, index) in listData" :key="index" :class="[componentContentClass]">
          ...
          <view :class="[ns.e('content')]">
            <view v-for="(dataItem, dataIndex) in item.data" :key="dataIndex" :class="[ns.e('content-item')]">
              <slot :data="dataItem" />
            </view>
          </view>
        </view>
    ...
    >
      <view v-for="(item, index) in keysData" :key="index" :class="[ns.e('key-list-value'), 'key-value']">
        {{ item }}
      </view>

      <!-- 提示框 -->
      <view v-if="currentTouchKeyIndex !== -1" :class="[ns.e('key-list-tips-value')]" :style="{ top: `${keyListTipsTopValue}px` }">
        {{ currentTouchKeyValue }}
...
@use '../../../theme-chalk/src/index-list.scss' as *;
</style>
