...
<template>
  <view :class="[ns.b()]">
    <view v-for="(item, index) in imageData" :key="index" :class="[ns.e('container')]" :style="containerStyle" @tap.stop="imageClickEvent(index)">
      <view :class="ns.e('item')">
        <TnLazyLoad v-if="lazyLoad" :src="item" :mode="props.imgMode" />
        <image v-else :class="ns.e('item__image')" :src="item" :mode="props.imgMode" />
      </view>
    </view>
  </view>
</template>
...
@use '../../../theme-chalk/src/photo-album.scss' as *;
</style>
