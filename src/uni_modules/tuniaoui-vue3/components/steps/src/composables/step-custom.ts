...
export const useStepCustomStyle = (props: StepProps, isActive: Ref<boolean>, stepMode: Ref<StepsMode>) => {
    const ns = useNamespace('step');

    const stepsContext = inject(stepsContextKey);

    const normalColor = computed<string | undefined>(() => props.color || stepsContext?.color);
    const activeColor = computed<string | undefined>(() => props.activeColor || stepsContext?.activeColor);

    // 解析颜色
    const [textColorClass, textColorStyle] = useComponentColor(normalColor, 'text');
    const [textActiveColorClass, textActiveColorStyle] = useComponentColor(activeColor, 'text');
    const [bgColorClass, bgColorStyle] = useComponentColor(normalColor, 'bg');
    const [bgActiveColorClass, bgActiveColorStyle] = useComponentColor(activeColor, 'bg');
    ...
    const stepStyle = computed<CSSProperties>(() => {
        ...
        // 设置颜色
        if (isActive.value) {
            if (!textActiveColorClass.value) style.color = textActiveColorStyle.value || 'var(--tn-color-primary)';
        } else {
            if (!textColorClass.value) style.color = textColorStyle.value || 'var(--tn-color-gray)';
        }
    ...
    const modeStyle = computed<StepModeStyle>(() => {
        return (selector?: ModeSelector) => {
            ...
            if (stepMode.value === 'dot') {
                if (isActive.value) {
                    if (!bgActiveColorClass.value) style.backgroundColor = bgActiveColorStyle.value || 'var(--tn-color-primary)';
                } else {
                    if (!bgColorClass.value) style.backgroundColor = bgColorStyle.value || 'var(--tn-color-gray)';
                }
            }
