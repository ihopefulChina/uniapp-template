...
export const useRate = (props: RateProps, emits: SetupContext<RateEmits>['emit']) => {
    ...
    const { currentX: rateCurrentX, updateOptions: updateRateTouchOptions, onTouchStart: rateTouchStartHandler, onTouchMove: rateTouchMoveHandler, onTouchEnd: rateTouchEndHandler } = useTouch();
    ...
    const getComponentRectInfo = async () => {
        try {
            const itemRectInfo = await getSelectorNodeInfo(`#${componentId} .tn-rate__item`);
