...
import type { CalendarEmits, CalendarModelValueType, CalendarProps } from '../calendar';
import type { CalendarData, CalendarItem, CalendarItemDateStatus, CalendarMonthData, CalendarRangeSelectData, CalendarSelectDataMap, DateData } from '../types';
...
// 根据年月日生成日期字符串
const generateDateStr = (date: DateData) => `${date.year}/${fillDateZero(date.month)}/${fillDateZero(date.date)}`;

export const useCalendar = (props: CalendarProps, emits: SetupContext<CalendarEmits>['emit']) => {
    // 日历的数据
    const calendarData = ref<CalendarData>([]);
    const { t } = useLocale();
    // 星期提示文字
    const weekText = ref<string[]>([t('common.sunDay'), t('common.monDay'), t('common.tuesDay'), t('common.wednesDay'), t('common.thursDay'), t('common.friDay'), t('common.saturDay')]);
    ...
    const updateModelValue = (changeEmit = true) => {
        ...
        switch (mode) {
            ...
            case 'multi':
                ...
                const activeIndex = (formatModelValue as string[]).indexOf(generateDateStr(currentSelectedDate));
                if (formatModelValue.length && activeIndex !== -1) {
                    ...
                    updateDateStatus(currentSelectedDate, 'active');
                    value = [...(formatModelValue as string[]), generateDateStr(currentSelectedDate)];
                }
                break;
            case 'range':
                ...
                if (!formatModelValue || formatModelValue.length === 0 || formatModelValue.length === 2) {
                    if (formatModelValue.length === 2) {
                        // 清空原来设置的时间
                        const startDateDayjs = dayjs((formatModelValue as string[])[0], DEFAULT_DATE_FORMAT);
                        const endDateDayjs = dayjs((formatModelValue as string[])[1], DEFAULT_DATE_FORMAT);
    ...
    const currentMonthIndex = computed(() => {
        return calendarData.value.findIndex((item) => item.month === currentSelectedDate.month);
    });
    // 当前月份中日期数据的长度
    ...
    // 处理容器信息
    const { calendarId, dateContainerHeight } = useCalendarSelector(currentMonthDateLength, props.mode);
    ...
    const generateCalendarData = () => {
        ...
        const _fillDateData = (month: number, minDisabledDate = 0, maxDisabledDate = 0) => {
            ...
            for (let i = 1; i <= days; i++) {
                let status: CalendarItemDateStatus = i < minDisabledDate || (maxDisabledDate && i > maxDisabledDate) ? 'disabled' : 'normal';
    ...
    // 根据传递的日期，设置当前日期的状态
    const updateDateStatus = (dateData: DateData | Dayjs, status: CalendarItemDateStatus) => {
        let month = 0;
        let date = 0;
        ...
        const dateIndex = calendarData.value[monthIndex].data.findIndex((item) => item.date === date);
    ...
    const dateItemClickEvent = (date: CalendarItem) => {
        if ((date.status === 'active' && props.mode === 'date') || date.status === 'disabled' || date.date === 0) return;
    ...
    const switchMonth = (type: 'prev' | 'next') => {
        ...
        if (type === 'prev') {
            if ((currentSelectedDate.year === minDate.year && currentSelectedDate.month === minDate.month) || currentSelectedDate.month === 1) return;
            currentSelectedDate.month--;
        } else {
            if ((currentSelectedDate.year === maxDate.year && currentSelectedDate.month === maxDate.month) || currentSelectedDate.month === 12) return;
            currentSelectedDate.month++;
        }
    };
