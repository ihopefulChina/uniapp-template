...
export const useFooter = (props: FooterProps, emits: SetupContext<FooterEmits>['emit']) => {
    // 导航数据
    const navigatorData = computed<FooterNavigatorData>(() => {
        return props.navigator.map((nav) => {
            const textColor = ref(isEmptyDoubleVariableInDefault(nav.textColor, props.navigatorTextColor));
            const [textColorClass, textColorStyle] = useComponentColor(textColor, 'text');
            return {
                title: nav.title || '',
                url: nav?.url || '',
                color: {
