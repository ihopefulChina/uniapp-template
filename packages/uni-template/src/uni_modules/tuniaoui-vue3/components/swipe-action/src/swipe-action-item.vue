...
const { componentId, optionComponentClass, optionsMenuWidth, contentRightDistance, isSwipe, onTouchStart, onTouchMove, onTouchEnd, optionClickEvent } = useSwipeActionItem(props);
const { options, optionColorClass, optionColorStyle } = useSwipeActionOptions(props);
</script>
  ...
  <view :id="componentId" :class="[ns.b()]">
    ...
    <view :class="[ns.e('options')]">
      <view
        ...
        :key="index"
        :class="[optionComponentClass, ns.e('option-item'), ns.is('disabled', item.disabled), ns.is('round', item.round)]"
        :style="{
      ...
      >
        <view :class="[ns.e('option-item-content'), optionColorClass(index)]" :style="optionColorStyle(index)">
...
@use '../../../theme-chalk/src/swipe-action-item.scss' as *;
</style>
