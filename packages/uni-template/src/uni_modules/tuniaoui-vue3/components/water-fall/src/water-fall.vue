...
<template>
  <view :class="[ns.b()]">
    ...
    <view :id="`${componentId}-left`" class="left" :class="[ns.e('container')]">
      <view v-for="(item, index) in processedLeftData" :key="index" :class="[ns.e('item')]">
    ...
    <!-- 右边数据 -->
    <view :id="`${componentId}-right`" class="right" :class="[ns.e('container')]">
      <view v-for="(item, index) in processedRightData" :key="index" :class="[ns.e('item')]">
        <template v-if="item.__is_insertion">
          <slot :name="item.slot" />
        </template>
        <template v-else>
...
@use '../../../theme-chalk/src/water-fall.scss' as *;
</style>
