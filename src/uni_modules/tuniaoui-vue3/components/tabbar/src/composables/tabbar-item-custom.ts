...
export const useTabbarItemCustomStyle = (props: TabbarItemProps, isActive: Ref<boolean>) => {
    const ns = useNamespace('tabbar-item');
    const tabbarContext = inject(tabbarContextKey);

    const activeColor = computed<string | undefined>(() => props.activeColor || tabbarContext?.activeColor);
    const inactiveColor = computed<string | undefined>(() => props.inactiveColor || tabbarContext?.inactiveColor);

    // 解析颜色
    const [inactiveColorClass, inactiveColorStyle] = useComponentColor(inactiveColor, 'text');
    const [activeColorClass, activeColorStyle] = useComponentColor(activeColor, 'text');
    const [inactiveBgClass, inactiveBgStyle] = useComponentColor(inactiveColor, 'bg');
    const [activebgClass, activebgStyle] = useComponentColor(activeColor, 'bg');
    const [bulgeBgClass, bulgeBgStyle] = useComponentColor(toRef(props, 'bulgeBgColor'), 'bg');
    const [bulgeTextColorClass, bulgeTextColorStyle] = useComponentColor(toRef(props, 'bulgeTextColor'), 'text');

    // 文字尺寸
    const fontSize = computed<string>(() => formatDomSizeValue(props.fontSize || tabbarContext?.fontSize || ''));
    ...
    const tabbarItemClass = computed<string>(() => {
        ...
        // 判断是否只有图标或者文字
        if ((props.icon && props.activeIcon && !props.text) || (props.text && !props.icon && !props.activeIcon)) cls.push(ns.is('only-element'));
    ...
    const tabbarItemStyle = computed<CSSProperties>(() => {
        ...
        // 设置文字，图标颜色
        if (isActive.value) {
            if (!activeColorClass.value) style.color = activeColorStyle.value || 'var(--tn-color-primary)';
        } else {
            if (!inactiveColorClass.value) style.color = inactiveColorStyle.value || 'var(--tn-color-gray)';
        }
