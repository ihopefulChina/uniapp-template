...
const { navbackButtonType, hasRightOperation, clickBackEvent, clickHomeEvent } = useNavbar(props);
const { ns, backNs, navBarInfo, navbarClass, navbarStyle, navbarBgClass, navbarBgStyle, navbarPlaceholderStyle, navbarWrapperStyle, backStyle, contentStyle, rightOperationStyle } =
    ...
    <view :class="[ns.e('wrapper')]" :style="navbarWrapperStyle">
      ...
      <view v-if="navbackButtonType !== 'none'" :class="[backNs.b()]" :style="backStyle">
        <slot name="back">
          ...
          <!-- 单图标 -->
          <view v-if="navbackButtonType === 'single'" :class="[backNs.e('single')]" @tap.stop="backEvent(props.backIcon ? 'back' : 'home')">
      ...
      <!-- 右边操作区域 -->
      <view v-if="$slots.right" :class="[ns.e('right-operation')]" :style="rightOperationStyle">
        <slot name="right" />
      </view>
    </view>
  </view>
...
@use '../../../theme-chalk/src/navbar.scss' as *;
</style>
