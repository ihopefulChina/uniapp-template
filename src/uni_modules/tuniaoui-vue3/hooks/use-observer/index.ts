...
export const useObserver = (instance?: ComponentInternalInstance | null) => {
    ...
    const connectObserver = (selector: string, fn: (res: UniApp.ObserveResult) => void, fnOptions: ObserverFnOptions, options?: UniApp.CreateIntersectionObserverOptions) => {
        // 开始监听布局之前先停止旧的监听
        disconnectObserver();

        observerInstance = uni.createIntersectionObserver(instance, options);
        if (fnOptions.type === 'relativeTo') observerInstance.relativeTo(fnOptions?.selector || '', fnOptions.margins);
        else if (fnOptions.type === 'relativeToViewport') observerInstance.relativeToViewport(fnOptions.margins);
