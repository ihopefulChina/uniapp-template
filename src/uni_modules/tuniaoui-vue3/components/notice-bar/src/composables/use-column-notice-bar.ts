...
export const useColumnNoticeBar = () => {
    ...
    // 通知数据
    const data = computed<string[]>(() => isEmptyVariableInDefault(noticeBar?.data, []));

    // 通知切换时间
    const interval = computed<number>(() => isEmptyVariableInDefault(noticeBar?.speed, 3000));

    // 是否播放
    const play = computed<boolean>(() => isEmptyVariableInDefault(noticeBar?.play, true));
