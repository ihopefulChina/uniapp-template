...
export const useNotifyCustomStyle = (props: NotifyProps, options: Ref<NotifyOptions>, isActive: Ref<boolean>) => {
    const ns = useNamespace('notify');

    // 解析颜色
    const [bgColorClass, bgColorStyle] = useComponentColor(toRef(options.value, 'bgColor'), 'bg');
    const [textColorClass, textColorStyle] = useComponentColor(toRef(options.value, 'textColor'), 'text');
