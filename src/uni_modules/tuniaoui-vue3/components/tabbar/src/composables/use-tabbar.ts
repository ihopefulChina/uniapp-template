...
export const useTabbar = (props: TabbarProps) => {
    ...
    const { children: items, addChild, removeChild: removeItem } = useOrderedChildren<TabbarItemContext>();
    ...
    const setActiveItem = (uid: number) => {
        ...
        const isPromiseOrBoolean = [isPromise(shouldSwitch), isBoolean(shouldSwitch)].includes(true);
