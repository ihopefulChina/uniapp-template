...
export const useNumberBoxCustomStyle = (props: NumberBoxProps, inputValue: Ref<number>) => {
    ...
    const numberBoxOperationWrapperStyle = computed<OperationWrapperStyle>(() => {
        return (type: OperationWrapperType) => {
            ...
            // 设置背景颜色和字体颜色
            if (!bgColorClass.value) style.backgroundColor = bgColorStyle.value || 'var(--tn-color-gray-light)';
